...
{ // v2.0
	...
	public static void main(String[] args)
	{
		...
		try(BufferedReader input = new BufferedReader(new InputStreamReader(System.in)))
		{
			...
			while((line = input.readLine()) != null)
			{
				...
				try
				{
					...
					assign("last = " + result); // Save it!
					System.out.printf("%.5f",result);
					System.out.println();
				}
				catch(DzeException | NumberFormatException ex)
				{
					debug(ex.getMessage());
		...
		catch(IOException | DzeException ex)
		{
			debug(ex.getMessage());
			System.out.println("ERROR");
	...
	public static void define(String line) throws DzeException
	{
		...
		while(i != line.length() - 1 && (c = line.charAt(i)) == ' ') // Proceed to the name.
			i++;
		
		while(i != line.length() - 1 && (c = line.charAt(i)) != '(')
		{
	...
	public static Stack<String> translate(Stack<String> stack) throws DzeException
	{
		...
		for(int i = 0; i < stack.size(); i++)
		{
			...
			if(variables.containsKey(temp))
				stack.setElementAt(variables.get(temp).toString(),i);
			//else if(!is_num(temp) && (temp.length() == 1 && "*+-/".indexOf(temp) < 0))
			else if(is_word(temp))
			{
				assign(temp + " = 0");
				stack.setElementAt("0",i);
			}
		}
		return stack;
	}
	...
	public static Stack<String> parse(String line) throws DzeException
	{
		...
		while(i < line.length())
		{
			...
			else if(c == '-' && (i == 0 || is_oper(line.charAt(i))))
			{
			...
			else if(Character.isWhitespace(c) && temp.length() != 0 && !temp.equals(""))
			{
				if(temp.length() > 0 && !temp.equals(" "))
					stack.push(temp.toString());
				temp = new StringBuilder("");
	...
	public static String translate_scientific(String line) throws DzeException
	{
		...
		while(temp.indexOf("e") > 0)
		{
			...
			debug("REPLACE: " + fnum + "e" + snum + " = " + Double.parseDouble(fnum + "e" + snum));

			// Replace it with the number.
			temp.replace(start,end + 1,Double.toString(Double.parseDouble(new String(fnum + "e" + snum))));
		}
